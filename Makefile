...
PROJECT_NAME=tremt
STACK_NAME=tremt-edgard-leonel-tangrama-com-br
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t edgard-leonel/tremt-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t edgard-leonel/tremt-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name tremt-frontend-acceptance --link tremt-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d edgard-leonel/tremt-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name tremt-backend-acceptance -d edgard-leonel/tremt-backend:acceptance
